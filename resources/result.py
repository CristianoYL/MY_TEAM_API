...
class ResultByTournamentClub(Resource):
    def get(self, tournamentID, clubID):  # get club's results
        return {'results': [result.json() for result in
                            ResultModel.find_club_tournament_result(tournamentID, clubID)]}, 200
    ...
    def post(self, tournamentID, clubID):  # post new game result and update player stats
        ...
        for stats in data['stats']:
            ...
            if not previous_stats:
                return {'message': 'Internal server error, failed to locate player <id:{}> stats'.format(
                    stats["playerID"])}, 500
